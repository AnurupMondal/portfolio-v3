...
const Navbar: React.FC = () => {
    ...
    const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, _path: string, id: string) => {
        e.preventDefault();
        setIsOpen(false);
    ...
    return (
        <nav
            className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-background/80 backdrop-blur-md border-b border-white/10 dark:border-white/10 border-black/5 py-2' : 'bg-transparent py-4'
                }`}
        >
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        {navLinks.map((link) => (
                            <a
                                ...
                                className={`text-sm font-medium transition-colors hover:text-primary relative ${activeId === link.id ? 'text-primary' : 'text-gray-600 dark:text-gray-300'
                                    }`}
                            >
                                {link.name}
                                {activeId === link.id && (
                        ...
                        <div className="flex items-center space-x-4 border-l border-white/10 pl-4">
                            ...
                            <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="text-gray-600 dark:text-gray-300 hover:text-primary transition-colors">
                                <Github size={20} />
                            </a>
                    ...
                    >
                        <div className="px-4 pt-2 pb-6 space-y-2">
                            ...
                            <div className="flex space-x-4 px-3 py-4 border-t border-gray-200 dark:border-white/10 mt-4">
                                <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="text-foreground dark:text-gray-300 hover:text-primary">
                                    <Github size={24} />
                                </a>
                                <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-foreground dark:text-gray-300 hover:text-primary">
                                    <Linkedin size={24} />
                                </a>
                            </div>
                        </div>
